...
class Template
{
	...
	public function build($view, $data = array(), $return = FALSE)
	{
		...
		if ($this->_layout)
		{
			...
			$template['menu'] = $this->_ci->Menumodel->buildMenu(0, "dropdown dropdown-horizontal", "dir");
			//$path = getcwd()."/themes/".get_theme()."/".$this->_layout. EXT;
	...
	private function _load_view($view, array $data, $parse_view = TRUE, $override_view_path = NULL)
	{
		...
		if ($override_view_path !== NULL)
		{
			...
			else
			{
				...
				$file = getcwd()."/themes/".get_theme()."/".$view.self::_ext($view);
...
//				$content = $this->_ci->load->file($override_view_path.$view.self::_ext($view), TRUE);
                                $content = $this->_ci->load->file($file, TRUE);
		...
		{
			$file = getcwd()."/themes/".get_theme()."/".$this->_module."/".$view.self::_ext($view);
			if (file_exists($file)) {
				$this->_ci->load->vars($data);
				$content = $this->_ci->load->file($file, TRUE);
			}
